#!/bin/bash
# ==============================================================================
# Step 5: Intensity Normalization
# ==============================================================================
# Normalize intensities to [0, 1] range using one of four methods.
# When a brain mask is provided, statistics are computed within the mask
# and non-brain voxels are zeroed out (matching SHiVAi convention).
#
# Usage: bash 05_normalize.sh <input> <output> [method] [brain_mask] [apply_mask]
#   <input>        Cropped image
#   <output>       Normalized image
#   [method]       min_max_masked (default) | max_norm | p1_p99 | min_max
#   [brain_mask]   Binary brain mask (optional — stats computed within mask)
#   [apply_mask]   true|false — multiply output by brain mask (default: true)
#
# Variants:
#   min_max_masked — (I - min_brain) / (max_brain - min_brain), clip [0,1], mask
#   max_norm       — I / P99, clip [0,1]                         (SHIVA)
#   p1_p99         — (I - P1) / (P99 - P1), clip [0,1]          (MUJICA)
#   min_max        — (I - min) / (max - min), clip [0,1]         (Standard ML)
# ==============================================================================

set -euo pipefail

INPUT="$1"
OUTPUT="$2"
METHOD="${3:-min_max_masked}"
BRAIN_MASK="${4:-}"
APPLY_MASK="${5:-true}"

if [[ ! -f "${INPUT}" ]]; then
    echo "ERROR: Input file not found: ${INPUT}"
    exit 1
fi

# Build mask flag for fslstats: -k <mask> restricts stats to mask voxels
MASK_FLAG=""
if [[ -n "${BRAIN_MASK}" && -f "${BRAIN_MASK}" ]]; then
    MASK_FLAG="-k ${BRAIN_MASK}"
    echo "  [Step 5] Normalizing (${METHOD}, brain-masked): $(basename ${INPUT})"
else
    echo "  [Step 5] Normalizing (${METHOD}): $(basename ${INPUT})"
fi

TMPFILE="${OUTPUT%.nii.gz}_tmp.nii.gz"

case "${METHOD}" in
    min_max_masked|min_max)
        RANGE=$(fslstats "${INPUT}" ${MASK_FLAG} -R)
        VMIN=$(echo "${RANGE}" | awk '{print $1}')
        VMAX=$(echo "${RANGE}" | awk '{print $2}')
        echo "    min=${VMIN}, max=${VMAX}"

        3dcalc -a "${INPUT}" \
            -expr "(a-${VMIN})/(${VMAX}-${VMIN})" \
            -prefix "${TMPFILE}" -nscale -float -overwrite

        3dcalc -a "${TMPFILE}" \
            -expr "a*within(a,0,1)" \
            -prefix "${OUTPUT}" -nscale -float -overwrite

        rm -f "${TMPFILE}"
        ;;

    max_norm)
        P99=$(fslstats "${INPUT}" ${MASK_FLAG} -P 99 | awk '{print $1}')
        echo "    P99=${P99}"

        3dcalc -a "${INPUT}" \
            -expr "(a)/(${P99})" \
            -prefix "${TMPFILE}" -nscale -float -overwrite

        3dcalc -a "${TMPFILE}" \
            -expr "a*within(a,0,1)" \
            -prefix "${OUTPUT}" -nscale -float -overwrite

        rm -f "${TMPFILE}"
        ;;

    p1_p99)
        P1=$(fslstats "${INPUT}" ${MASK_FLAG} -P 1 | awk '{print $1}')
        P99=$(fslstats "${INPUT}" ${MASK_FLAG} -P 99 | awk '{print $1}')
        echo "    P1=${P1}, P99=${P99}"

        3dcalc -a "${INPUT}" \
            -expr "(a-${P1})/(${P99}-${P1})" \
            -prefix "${TMPFILE}" -nscale -float -overwrite

        3dcalc -a "${TMPFILE}" \
            -expr "a*within(a,0,1)" \
            -prefix "${OUTPUT}" -nscale -float -overwrite

        rm -f "${TMPFILE}"
        ;;

    *)
        echo "ERROR: Unknown normalization method: ${METHOD}"
        echo "  Options: min_max_masked, max_norm, p1_p99, min_max"
        exit 1
        ;;
esac

# Zero out non-brain voxels by multiplying with brain mask
if [[ "${APPLY_MASK}" == "true" && -n "${BRAIN_MASK}" && -f "${BRAIN_MASK}" ]]; then
    echo "    Applying brain mask to zero out non-brain voxels..."
    MASKED_TMP="${OUTPUT%.nii.gz}_masked_tmp.nii.gz"
    3dcalc -a "${OUTPUT}" -b "${BRAIN_MASK}" \
        -expr 'a*b' \
        -prefix "${MASKED_TMP}" -nscale -float -overwrite
    mv "${MASKED_TMP}" "${OUTPUT}"
fi

echo "  [Step 5] Done: $(basename ${OUTPUT})"
